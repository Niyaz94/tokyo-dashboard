            ...
            <Box width={300} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <FormControl fullWidth variant="outlined">
                ...
                <Select value={filters.status || 'all'} onChange={handleStatusChange} label="Status" autoWidth>
                  {unique_type.map((name) => (<MenuItem key={name} value={name}>{name}</MenuItem>))}
                </Select>
              </FormControl>
            </Box>
          }
        ...
        <Table>
          ...
          <TableBody>
            ...
            {paginatedData.map((row) => {
                return <CustomTableRow data={row} unique_type={unique_type} isDataSelected={selectedData.includes(row.id)} handleSelectOneData={handleSelectOneData}/>
            })}
          </TableBody>
        </Table>
      </TableContainer>
