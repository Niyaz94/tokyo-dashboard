...
const DataTable = () => {
  ...
  const { page, limit, handlePageChange, handleLimitChange } = useTablePaginationHandlers('documentType');
  ...
  const deleteSelectedRows = async () => {
    deleteTableMultiRow(selectedIds,"document/document/type/multi_delete/",setTable)
    setSelectedIds([]);
  }
  return (
    <Card>
        ...
        <CardHeader
          ...
          action={
            <Box width={600} height={70} sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'flex-end' }}>
              <Tooltip title="Add New Document Type" sx={{fontSize: '0.8rem'}} placement="top">
                <Button
      ...
      <SelectableTable
        ...
        renderRow={(row) => (
          <CustomTableRow
            ...
            handleSelectOneData={handleSelectOne} onDeleteRow={async ()=>deleteTableRow(row.id,"document/document/type",setTable)}
          />
        )}
      />
      <CustomPagination
