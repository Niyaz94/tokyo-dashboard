...
const DataTable = () => {
  ...
  const { page, limit, handlePageChange, handleLimitChange } = useTablePaginationHandlers('passwordType');
  ...
  const deleteSelectedRows = async () => {
    deleteTableMultiRow(selectedIds,"document/password/type/multi_delete/",setTable)
    setSelectedIds([]);
  }
  return (
    <Card>
        ...
        <CardHeader
          ...
          action={
            <Box width={600} height={70} sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'flex-end' }}>
              <Tooltip title="Add New Password Type" sx={{fontSize: '0.8rem'}} placement="top">
                <Button
      ...
      <SelectableTable
        ...
        renderRow={(row) => (
          <CustomTableRow
            ...
            handleSelectOneData={handleSelectOne} onDeleteRow={async ()=>deleteTableRow(row.id,"document/password/type",setTable)}
          />
        )}
      />
      <CustomPagination
