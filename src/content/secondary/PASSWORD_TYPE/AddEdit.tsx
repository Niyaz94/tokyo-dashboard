...
const CollapsibleForm = () => {
  ...
  const { data:fetchEditData,success:editReturnSuccess}: FetchData<FormIntialStateInterface>  = useFetch <FormIntialStateInterface>(edit_page_id ?`document/password/type/${edit_page_id}`: null,{});
  ...
  const handleSave = async () => {
    ...
    if (edit_page_id) {
      await editData(`document/password/type/${edit_page_id}/`, formData);
    }else{
      await postData("document/password/type/", dataToBeSent,"FORM");
    }
  };
  ...
  return (
      <Card>
        <CardHeader title="Password Type" />
        <Divider />
        <CardContent>
