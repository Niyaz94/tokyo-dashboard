...
export default () =>{
  ...
  const { data,success}: FetchData<TopicSingleSampleInterface[]> = useFetch <TopicSingleSampleInterface[]>('document/learn',[]);

   const { data:unique_data,success:unque_success}: FetchData<{type:string[]}> = useFetch <{type:string[]}>('document/learn/unique',{
      type:[]
    });
  ...
  return (
    <PageProvider tableData={data} secondaryData={unique_data}>
      <Template templateTitle="Documents - Learn">
        <Routes>
