...
import {columnsLearn as columns} from '../../../utility/function/tableColumn';
...
const DataTable = () => {
  ...
  const { page, limit, handlePageChange, handleLimitChange } = useTablePaginationHandlers('learn');
  ...
  const { paginatedData, filteredData } = useStaticTableFilters<Omit<SingleSampleInterface,'secrecy_level'>>(tableData,filters,page,limit);

  const deleteSelectedRows = async () => {
    deleteTableMultiRow(selectedIds,"document/learn/multi_delete/",setTable)
    setSelectedIds([]);
  }
        ...
        <CardHeader
          ...
          action={
            <Box width={600} height={70} sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'flex-end' }}>
              ...
              <Tooltip title="Add New Learn Title" sx={{fontSize: '0.8rem'}} placement="top">
                <Button
                  variant="outlined" color="error" onClick={() => navigate('/documents/learn_type/add',{state: { from: location.pathname } })}
              ...
              <Tooltip title="Add New Learn" sx={{fontSize: '0.8rem'}} placement="top">
                <Button
      ...
      <SelectableTable
        ...
        renderRow={(row) => (
          <CustomTableRow
            ...
            handleSelectOneData={handleSelectOne} onDeleteRow={async ()=>deleteTableRow(row.id,"document/learn",setTable)}
          />
        )}
      />
      <CustomPagination
