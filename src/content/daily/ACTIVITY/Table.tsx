...
import {columnsActivity as columns} from '../../../utility/function/tableColumn';
...
const DataTable = () => {
  ...
  useEffect(() => {
      axiosGetData(`notes/activity/?page=${page+1}&page_size=${limit}`,loginDetail.token).then((res) => {
      ...
      <SelectableTable
        ...
        renderRow={(row) => (
          <CustomTableRow
            ...
            handleSelectOneData={handleSelectOne} onDeleteRow={async ()=>deleteTableRow(row.id,"notes/activity",setTable)}
          />
        )}
      />
      <CustomPagination
